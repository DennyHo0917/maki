...
/// result. Both authorings go through [`Self::known_levels`], the one place a
/// level maki has no name for is dropped.
...
impl ModelEffort {
    ...
    /// What [`Self::supported`] keeps of the names a provider listed, for a
    /// Rust parser that builds the row itself rather than decoding it.
    pub fn known_levels(listed: &[Value]) -> Vec<Effort> {
